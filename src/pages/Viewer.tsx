...
// تأكد أن اسم الملف هو ThemeVoiceGift1 أو حسب اللي عندك
...
export default function Viewer() {
  ...
  useEffect(() => {
    ...
    const processGift = async () => {
      try {
        ...
        // 🌟 التعديل هنا: منع الثيمات الـ VIP من الدخول في iframe (الروح الواحدة ونبض الصوت) 🌟
        if (themeSlug !== 'one-soul' && themeSlug !== 'voice-gift1') {
  ...
  if (error === 'blocked') return ( <div style={pageContainer}><div style={{...cardStyle, borderTop: '5px solid #dc2626'}}><img src="/oomniah-logo.png" alt="أمنية" style={logoStyle} /><FaBan style={{ fontSize: '50px', color: '#dc2626', marginBottom: '15px' }} /><h2 style={{ color: '#1e293b', fontSize: '24px', marginBottom: '10px', fontWeight: '900' }}>تم إيقاف الرابط</h2><p style={{ color: '#64748b', fontSize: '15px', lineHeight: '1.6', marginBottom: '25px' }}>عذراً، تم إيقاف هذا الرابط من قبل الإدارة المركزية.</p><a href={OOMNIAH_INSTAGRAM} target="_blank" rel="noreferrer" style={actionBtnStyle}><FaHeadset /> مراسلة الدعم الفني</a></div></div> );
  if (error === 'expired') return ( <div style={pageContainer}><div style={{...cardStyle, borderTop: '5px solid #ea580c'}}><img src="/oomniah-logo.png" alt="أمنية" style={logoStyle} /><FaHourglassEnd style={{ fontSize: '50px', color: '#ea580c', marginBottom: '15px' }} /><h2 style={{ color: '#1e293b', fontSize: '24px', marginBottom: '10px', fontWeight: '900' }}>انتهت صلاحية الهدية!</h2><p style={{ color: '#64748b', fontSize: '15px', lineHeight: '1.6', marginBottom: '25px' }}>عذراً، الوقت المخصص لعرض هذه الهدية قد انتهى.</p><a href={OOMNIAH_INSTAGRAM} target="_blank" rel="noreferrer" style={{...actionBtnStyle, background: 'linear-gradient(45deg, #ea580c, #f97316)', boxShadow: '0 10px 25px rgba(234, 88, 12, 0.3)'}}><FaSyncAlt /> طلب تجديد الرابط</a></div></div> );
  if (error === 'not_found' || error === 'unexpected') return ( <div style={pageContainer}><div style={cardStyle}><img src="/oomniah-logo.png" alt="أمنية" style={logoStyle} /><FaExclamationTriangle style={{ fontSize: '50px', color: '#1e293b', marginBottom: '15px' }} /><h2 style={{ color: '#1e293b', fontSize: '24px', marginBottom: '10px', fontWeight: '900' }}>الرابط غير صحيح</h2><p style={{ color: '#64748b', fontSize: '15px', lineHeight: '1.6', marginBottom: '25px' }}>عذراً، لا يمكننا العثور على هذه الهدية. قد يكون الرابط خاطئاً أو تم حذفه.</p><a href={window.location.origin} style={{...actionBtnStyle, background: 'linear-gradient(45deg, #1e293b, #334155)', boxShadow: '0 10px 25px rgba(30, 41, 59, 0.3)'}}><FaRocket /> تصفح خدمات منصة أمنية</a></div></div> );

  if (isOpened) {
    // تشغيل ثيم الروح الواحدة
    if (themeSlug === 'one-soul' && orderData) {
      return (
        <ThemeOneSoul 
          roomId={shortId} 
          orderData={{
            ...
            audio_url: orderData.song_url
          }} 
        />
      );
    }
    
    // 🌟 تشغيل ثيم النبض الصوتي (بالـ Slug الصحيح voice-gift1) 🌟
    if (themeSlug === 'voice-gift1' && orderData) {
      return (
        <ThemeVoiceGift 
          roomId={shortId} 
          orderData={{
   ...
   if (readyUrl) {
      return (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: '#050a15', zIndex: 999999, margin: 0, padding: 0, overflow: 'hidden' }}>
          <iframe src={readyUrl} style={{ width: '100%', height: '100%', border: 'none', display: 'block', margin: 0, padding: 0 }} allow="autoplay; fullscreen" title="Oomniah Gift" />
        </div>
      );
    }
  }

  // 🌟 التعديل هنا ليتوافق مع voice-gift1
  if (!isLoaded || (!readyUrl && themeSlug !== 'one-soul' && themeSlug !== 'voice-gift1')) return (
  ...
  return (
    <div style={pageContainer}>
      <style>{`@keyframes float { 0% { transform: translateY(0px); } 50% { transform: translateY(-15px); } 100% { transform: translateY(0px); } } @keyframes pulse-ring { 0% { box-shadow: 0 0 0 0 rgba(220, 38, 38, 0.4); } 70% { box-shadow: 0 0 0 20px rgba(220, 38, 38, 0); } 100% { box-shadow: 0 0 0 0 rgba(220, 38, 38, 0); } } .pulse-btn { animation: pulse-ring 2s infinite; } .hover-effect:hover { transform: translateY(-3px) scale(1.03); } .background-blob { position: absolute; filter: blur(60px); opacity: 0.4; z-index: 1; border-radius: 50%; }`}</style>
      ...
      <div style={cardStyle} className="fade-in">
        <img src="/oomniah-logo.png" alt="أمنية" style={{ width: '100px', height: '100px', animation: 'float 4s ease-in-out infinite', objectFit: 'contain', marginBottom: '20px', filter: 'drop-shadow(0 15px 25px rgba(220,38,38,0.25))' }} onError={(e) => { e.currentTarget.style.display = 'none' }} />
        ...
        {isInAppBrowser ? (
          <div style={{ textAlign: 'center', padding: '25px 20px', background: '#fff0f2', borderRadius: '20px', border: '1px dashed #fecaca', width: '100%' }}>
            ...
            <p style={{ color: '#475569', fontSize: '13px', lineHeight: '1.6', marginBottom: '20px', fontWeight: 'bold' }}>أنت تستخدم متصفح تطبيق. لضمان تجربة المفاجأة كاملة مع الصوت، يرجى فتح الرابط في المتصفح الخارجي.</p>
            <button onClick={forceOpenExternalBrowser} style={{...actionBtnStyle, marginTop: 0, padding: '14px 20px', fontSize: '15px'}} className="hover-effect">فتح في المتصفح الأساسي <FaGlobe style={{ fontSize: '18px' }} /></button>
            <button onClick={handleOpenGift} style={{ background: 'transparent', color: '#94a3b8', border: 'none', marginTop: '15px', fontSize: '13px', textDecoration: 'underline', cursor: 'pointer', fontWeight: 'bold' }}>تخطي وفتح الهدية هنا (بدون موسيقى)</button>
          </div>
        ) : (
      ...
      {/* 🌟 التعديل هنا ليتوافق مع voice-gift1 */}
      {!isOpened && readyUrl && themeSlug !== 'one-soul' && themeSlug !== 'voice-gift1' && (
