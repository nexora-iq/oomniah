...
export default function POS() {
  ...
  if (formData.duration_type === 'trial') {
    ...
    if (selectedTheme?.is_multiplayer) {
      ...
      originalBasePrice = durationBasePrice;
    }
  }
  ...
  return (
    <div className="pos-page-wrapper">
      <style>{`
        ...
        .interactive-card { border: 2px solid #e2e8f0; background: #fff; padding: 12px 6px; border-radius: 12px; text-align: center; cursor: pointer; transition: all 0.2s; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 6px; min-height: 80px; position: relative; }
            ...
            {Object.entries(DURATION_PRICES).map(([key, val]) => {
              ...
              return (
                <div key={key} className={`interactive-card ${formData.duration_type === key ? 'active' : ''}`} onClick={() => setFormData({...formData, duration_type: key as DurationType})} style={key === 'trial' ? { borderStyle: 'dashed' } : {}}>
                  ...
                  <span style={{ fontSize: '11px', color: formData.duration_type === key ? '#fff' : '#64748b', fontWeight: 'bold' }}>
                    {key === 'trial' ? 'مجاني' : `${displayPrice.toLocaleString()} د.ع`}
                  </span>
                </div>
              );
            })}
