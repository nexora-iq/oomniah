...
import { 
  ...
  FaUserTie, FaBan, FaCheckCircle, FaSpinner 
} from 'react-icons/fa';
...
export default function BranchesManager() {
  ...
  const fetchBranchesAndEmployees = async () => {
    ...
    try {
      ...
      const { data: profilesData, error: profilesError } = await supabase.from('profiles').select('id, fullname, role, page_id, is_blocked').neq('role', 'super_admin');
      ...
      const branchesWithEmployees = (pagesData || []).map(branch => {
        ...
        return { ...branch, employees: branchEmployees };
      });

      setBranches(branchesWithEmployees);
    } catch (error: any) {
      Toast.fire({ icon: 'error', title: 'حدث خطأ أثناء الجلب.' });
    } finally {
  ...
  const toggleStatus = async (id: string, current: string, branchName: string) => {
    ...
    if (error) {
      ...
      Toast.fire({ icon: 'success', title: newStatus === 'active' ? `تم تفعيل ${branchName}` : `تم إيقاف ${branchName}` });
    }
  };
  ...
  return (
    <div className="fade-in">
      <style>{`
        ...
        .input-style { padding: 12px 15px; border-radius: 10px; border: 1px solid #cbd5e1; width: 100%; box-sizing: border-box; background: #f8fafc; font-size: 14px; transition: 0.3s; }
        ...
        .big-spinner { width: 40px; height: 40px; border: 4px solid #f8fafc; border-top: 4px solid #dc2626; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto; }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
      `}</style>

      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
        <div>
          <h2 style={{ margin: '0 0 5px 0', color: '#1e293b', fontSize: '24px', fontWeight: '900', display: 'flex', alignItems: 'center', gap: '10px' }}><FaBuilding style={{ color: '#dc2626' }} /> إدارة الفروع (SaaS)</h2>
          <p style={{ color: '#64748b', fontSize: '14px', margin: 0 }}>أضف فروعاً جديدة وتابع فريق العمل الخاص بكل فرع بمرونة تامة.</p>
        </div>
        <div style={{ background: '#fef2f2', color: '#dc2626', padding: '10px 20px', borderRadius: '12px', fontWeight: 'bold' }}>إجمالي الفروع: {branches.length}</div>
      </div>
      ...
      <form onSubmit={handleAdd} className="card" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', alignItems: 'end' }}>
        ...
        <div>
          ...
          <select value={type} onChange={e => { setType(e.target.value); setOwnerPercentage(e.target.value === 'owned' ? '100' : '50'); }} className="input-style">
            <option value="owned">مملوك لأمنية (100%)</option>
            <option value="contract">متعاقد (نسبة مئوية)</option>
          </select>
        </div>
        <div>
        ...
        <button type="submit" disabled={loading} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: loading ? '#fca5a5' : '#dc2626', color: '#fff', padding: '12px', borderRadius: '10px', border: 'none', fontWeight: 'bold', cursor: loading ? 'not-allowed' : 'pointer', height: '43px', transition: '0.3s' }}>
          {loading ? <><FaSpinner className="spinner" /> جاري الإضافة...</> : <><FaPlus /> تسجيل الفرع</>}
        </button>
      </form>
      ...
      {dataLoading ? (
        <div style={{ padding: '60px', textAlign: 'center' }}>
          ...
          <p style={{ marginTop: '15px', color: '#dc2626', fontWeight: 'bold' }}>جاري التحميل...</p>
        </div>
      ) : (
            ...
            <div key={b.id} className="branch-card" style={{ opacity: b.status === 'active' ? 1 : 0.7, filter: b.status === 'active' ? 'none' : 'grayscale(0.5)' }}>
              <div style={{ padding: '20px', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ background: '#f8fafc', padding: '4px 8px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold', border: '1px solid #e2e8f0', direction: 'ltr' }}>/{b.slug}</span>
                </div>
              </div>

              <div style={{ padding: '20px', background: '#fcfcfc', flexGrow: 1 }}>
                <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#475569', marginBottom: '10px' }}>فريق العمل ({b.employees?.length || 0}):</div>
                {b.employees && b.employees.length > 0 ? (
                  ...
                  <div style={{ color: '#94a3b8', fontSize: '13px', fontStyle: 'italic' }}>لا يوجد موظفين.</div>
                )}
              </div>

              <div style={{ padding: '15px 20px', background: '#fff', borderTop: '1px solid #f1f5f9' }}>
                <button onClick={() => toggleStatus(b.id, b.status, b.name)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '10px', borderRadius: '10px', border: 'none', background: b.status === 'active' ? '#fef2f2' : '#f0fdf4', color: b.status === 'active' ? '#dc2626' : '#16a34a', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' }}>
